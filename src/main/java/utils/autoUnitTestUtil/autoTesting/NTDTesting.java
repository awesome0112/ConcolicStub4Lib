...
import controller.ConcolicWithStubController;
...
public class NTDTesting {
    ...
    public static ConcolicTestResult runFullConcolic(String path, String methodName, String className, ConcolicWithStubController.Coverage coverage) throws IOException, NoSuchMethodException, InvocationTargetException, IllegalAccessException, ClassNotFoundException, NoSuchFieldException, InterruptedException {
    ...
    private static ConcolicTestResult startGenerating(ConcolicWithStubController.Coverage coverage) throws InvocationTargetException, IllegalAccessException, ClassNotFoundException, NoSuchFieldException {
    ...
    private static CfgNode findUncoverNode(CfgNode cfgNode, ConcolicWithStubController.Coverage coverage) {
        switch (coverage) {
            case STATEMENT:
    ...
    private static double calculateFullTestSuiteCoverage() throws ClassNotFoundException, NoSuchFieldException, IllegalAccessException {
        int totalFunctionStatement = (int) Class.forName(fullyClonedClassName).getField(getTotalFunctionCoverageVariableName((MethodDeclaration) testFunc, ConcolicWithStubController.Coverage.STATEMENT)).get(null);
        int totalCovered = MarkedPath.getFullTestSuiteTotalCoveredStatements();
        return (totalCovered * 100.0) / totalFunctionStatement;
    }

    private static double calculateRequiredCoverage(ConcolicWithStubController.Coverage coverage) throws ClassNotFoundException, NoSuchFieldException, IllegalAccessException {
        ...
        int totalCovered = 0;
        if (coverage == ConcolicWithStubController.Coverage.STATEMENT) {
            totalCovered = MarkedPath.getTotalCoveredStatement();
        } else if (coverage == ConcolicWithStubController.Coverage.BRANCH) {
    ...
    private static void setupCfgTree(ConcolicWithStubController.Coverage coverage) {
    ...
    private static ASTHelper.Coverage getCoverageType(ConcolicWithStubController.Coverage coverage) {
        switch (coverage) {
            case STATEMENT:
    ...
    private static String getTotalFunctionCoverageVariableName(MethodDeclaration methodDeclaration, ConcolicWithStubController.Coverage coverage) {
        StringBuilder result = new StringBuilder();
